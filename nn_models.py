...
class MLP1(torch.nn.Module):
    ...
    def __init__(self, input_dim, output_dim, bias_bool=True):
        super(MLP1, self).__init__()
        self.md_ls = []
        for i in range(5):
            self.md_ls.append(torch.nn.Linear(input_dim // 2, output_dim))

        for l in self.md_ls:
            torch.nn.init.orthogonal_(l.weight)  # use a principled initialization

    def forward(self, q_p, separate_fields=False):
        q = q_p[:, 0:3]
        p = q_p[:, 3:6]
        ls = [q, p, p * q, p * p, q * q]
        sum_ls = []
        for i in range(5):
            sum_ls.append(self.md_ls[i](ls[i]))
        res = torch.stack(sum_ls, dim=0)
