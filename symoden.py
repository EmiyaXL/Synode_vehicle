...
class SymODEN_R(torch.nn.Module):
    ...
    def forward(self, t, x):
        with torch.enable_grad():
            ...
            #     u_vec = u_tmp[u_order, :, :]
            # else:
            #     u_vec = test_u[u_order, :, :]
            u = self.u
            # print(u_order, x.shape)
            ...
            if self.baseline:
                ...
                return torch.cat((dq, dp, u), dim=1)
            if self.structure:
            ...
            # print(q_p[0, :], H.sum(), dH[0, :], H[0, :]*1e7)
            H_vector_field = torch.matmul(dH, self.M.t())
            # weight = torch.tensor([1000000000000000,10000000000,10000000000,1,1,1])
            # H_vector_field = weight * H_vector_field
            ...
            # F_vector_field = torch.cat((torch.zeros_like(F), F), dim=1)
            # print(H_vector_field.shape,F_vector_field.shape)
            # print(H_vector_field)
            # print('+++++')
            # print(dH, H_vector_field, F_vector_field)
            # print('+++++')
